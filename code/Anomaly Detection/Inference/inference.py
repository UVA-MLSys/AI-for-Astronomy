...
from Plot_Redshift import system_info
...
def inference(model, dataloader, real_redshift, plot_to_save_path, device, batch_size):
    ...
    if device == 'cuda':
        total_time = total_gpu_memory
    else:
        total_time = total_cpu_memory
    ...
    with open(plot_to_save_path + '_system_info.json', 'w') as json_file:
...
def engine(args):
    ...
    inference(model, dataloader, data[:][2], args.plot_path, device = args.device, batch_size = args.batch_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type = str, default = '/scratch/aww9gh/Cosmic_Cloud/AI-for-Astronomy/data/fraction_data/0.1189.pt')
    ...
    parser.add_argument('--plot_path', type = str, default = prj_dir + 'Plots/32_1')
